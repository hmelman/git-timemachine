...
(defface git-timemachine-minibuffer-detail
  '((t (:inherit warning)))
  "How to display the minibuffer detail"
  :group 'git-timemachine)

(defcustom git-timemachine-minibuffer-details
  'subject
...
(defun git-timemachine--revisions ()
  ...
  (or git-timemachine--revisions-cache
      (setq git-timemachine--revisions-cache
		...
		(let ((default-directory git-timemachine-directory)
		      ...
		      (status 0))
		  (with-temp-buffer
		    (setq status (process-file vc-git-program nil t nil
					       "--no-pager" "log" "--name-only" "--follow"
					       "--pretty=format:%H%x00%ar%x00%ad%x00%s" file))
		    (if (not (zerop status))
			(error "Git log command on file %s exited with non-zero exit status: %d" file status))
		    (goto-char (point-min))
		    (let ((lines)
  ...
  (when revision
	...
	(setq mode-line-buffer-identification
	      (list (propertized-buffer-identification "%12b") "@"
		    (propertize (git-timemachine-abbreviate commit) 'face 'git-timemachine-commit)
		    " name:" revision-file-name" " n-of-m)))
...
(defun git-timemachine--show-minibuffer-details (revision)
  ...
  (let ((detail
	 (if (eq git-timemachine-minibuffer-details 'commit)
	     (car revision)
	   (nth 5 revision)))
	(date-relative (nth 3 revision))
	(date-full (nth 4 revision)))
    (message (format "%s [%s (%s)]" (propertize detail 'face 'git-timemachine-minibuffer-detail) date-full date-relative))))
